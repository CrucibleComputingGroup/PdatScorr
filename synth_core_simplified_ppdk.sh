...
# (PPDK variant - uses PPDK Standard Library for gate-level synthesis)
#
# Supports multiple cores via YAML config files (Ibex, BOOM, Rocket, CVA6, etc.)
# Usage: ./synth_core_simplified_ppdk.sh [OPTIONS] <rules.dsl> [output_dir]
...
if [ "$SYNTHESIZE_GATES" = true ]; then
    ...
    if [ -n "$OPTIMIZED_BASE" ] && [ -f "$OPTIMIZED_SYNTH_DIR/${OPTIMIZED_BASE}_post_abc.aig" ]; then
        ...
        ./scripts/synth_to_gates_ppdk.sh "$OPTIMIZED_BASE_PATH" "" "$CLK_NAME" "$MODULE_NAME"
        ...
        ./scripts/synth_to_gates_ppdk.sh "$BASE" "" "$CLK_NAME" "$MODULE_NAME"
    ...
    echo "  ./scripts/synth_to_gates_ppdk.sh $BASE \"\" \"$CLK_NAME\" \"$MODULE_NAME\""
