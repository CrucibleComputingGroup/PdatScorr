...
# Function to run synthesis in background
run_synthesis_bg() {
    local script="$1"
    local abc_depth="$2"
    ...
    # Run in subshell to capture all output
    (
        local start_time=$(date +%s)

        echo "[$(date +%H:%M:%S)] Starting: $type_label with ABC depth=$abc_depth" >> "$output_subdir/timing.log"
        ...
        if timeout 600 "$script" --gates --abc-depth "$abc_depth" $extra_args "$DSL_FILE" "$output_subdir" > "$output_subdir/synthesis.log" 2>&1; then
            ...
            echo "[$(date +%H:%M:%S)] Completed: $type_label (depth=$abc_depth) in ${duration}s" >> "$output_subdir/timing.log"
            ...
            if [ -n "$pre_gates" ] && [ -n "$post_gates" ] && [ "$pre_gates" -gt 0 ]; then
                ...
                echo "  Reduction: ${reduction}%" >> "$output_subdir/timing.log"
            fi
        else
            echo "[$(date +%H:%M:%S)] FAILED: $type_label (depth=$abc_depth)" >> "$output_subdir/timing.log"
            echo "$type_label,$abc_depth,$pipeline,ERROR,ERROR,ERROR,ERROR,ERROR,FAILED" > "$result_file"
        fi
    ) &
}
...
# Launch all synthesis jobs in parallel
job_counter=0
for depth in "${ABC_DEPTHS[@]}"; do
    ...
    if [ -f "./synth_ibex_with_constraints.sh" ]; then
        ...
        run_synthesis_bg "./synth_ibex_with_constraints.sh" "$depth" "$output_dir" \
                        "ISA_only" "$result_file" "2stage"
        pids+=($!)
        job_labels+=("ISA_only_d${depth}")
        ((job_counter++))
    fi
    ...
    if [ -f "./synth_ibex_with_isa_and_timing.sh" ]; then
        ...
        run_synthesis_bg "./synth_ibex_with_isa_and_timing.sh" "$depth" "$output_dir" \
                        "ISA+timing" "$result_file" "2stage"
        pids+=($!)
        job_labels+=("ISA+timing_d${depth}")
        ((job_counter++))
    fi
    ...
    if [ "$depth" -ge 3 ]; then
        if [ -f "./synth_ibex_with_constraints.sh" ]; then
            ...
            pids+=($!)
            job_labels+=("ISA_only_3s_d${depth}")
            ((job_counter++))
        fi
        ...
        if [ -f "./synth_ibex_with_isa_and_timing.sh" ]; then
            ...
            pids+=($!)
            job_labels+=("ISA+timing_3s_d${depth}")
            ((job_counter++))
        fi
    fi
done

echo "Launched $job_counter parallel synthesis jobs"
...
while [ $finished -lt $total ]; do
    ...
    for i in "${!pids[@]}"; do
        if ! kill -0 "${pids[$i]}" 2>/dev/null; then
            ...
            running="$running ${job_labels[$i]}"
        fi
    done

    echo -ne "\rProgress: $finished/$total completed. Running:$running                    "
