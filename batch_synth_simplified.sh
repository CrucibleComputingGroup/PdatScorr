...
if [ ! -f "./synth_core_simplified.sh" ]; then
    echo -e "${RED}Error: synth_core_simplified.sh not found in current directory${NC}"
    exit 1
fi
...
if [ "$CLEAN_OUTPUT" = true ] && [ -d "$BASE_OUTPUT_DIR" ]; then
    echo -e "${YELLOW}Cleaning output directory: $BASE_OUTPUT_DIR${NC}"
...
run_synthesis() {
    ...
    if [ "$VERBOSE" = true ]; then
        ...
        ./synth_core_simplified.sh $EXTRA_ARGS "$dsl_file" "$parent_dir" 2>&1 | tee "$log_file"
        local exit_code=${PIPESTATUS[0]}
    else
        # Redirect to log file
        ./synth_core_simplified.sh $EXTRA_ARGS "$dsl_file" "$parent_dir" > "$log_file" 2>&1
        local exit_code=$?
    fi
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        # Show key metrics based on what was run
        # Check ODC-optimized results if --odc-analysis was used
        if [ "$HAS_ODC_FLAG" = true ] && [ -f "$actual_output_dir/odc_optimized_synthesis/abc.log" ]; then
            local stats=$(grep "i/o =" "$actual_output_dir/odc_optimized_synthesis/abc.log" | tail -1)
            if [ ! -z "$stats" ]; then
                echo "  └─ ODC-optimized: $stats"
            fi
        elif [ -f "$actual_output_dir/ibex_optimized_abc.log" ]; then
            local stats=$(grep "i/o =" "$actual_output_dir/ibex_optimized_abc.log" | tail -1)
            if [ ! -z "$stats" ]; then
                echo "  └─ Final: $stats"
            fi
        fi

        # Store total chip area if --gates was used
        if [ "$HAS_GATES_FLAG" = true ]; then
            if [ "$HAS_ODC_FLAG" = true ]; then
                # Check for ODC-optimized total_area.txt file first
                local odc_area_file=$(ls "$actual_output_dir/odc_optimized_synthesis/"*"_optimized_total_area.txt" 2>/dev/null | head -1)
                if [ -f "$odc_area_file" ]; then
                    cat "$odc_area_file" > "$actual_output_dir/area.txt" 2>/dev/null || true
                elif [ -f "$actual_output_dir/ibex_optimized_total_area.txt" ]; then
                    # Fallback to regular file if ODC didn't produce optimized version
                    cat "$actual_output_dir/ibex_optimized_total_area.txt" > "$actual_output_dir/area.txt" 2>/dev/null || true
                fi
            else
                # Use regular total_area.txt file
                if [ -f "$actual_output_dir/ibex_optimized_total_area.txt" ]; then
                    cat "$actual_output_dir/ibex_optimized_total_area.txt" > "$actual_output_dir/area.txt" 2>/dev/null || true
                fi
            fi

            # Store frequency if timing metrics available
            if [ "$HAS_ODC_FLAG" = true ]; then
                local timing_json=$(ls "$actual_output_dir/odc_optimized_synthesis/"*"_timing_metrics.json" 2>/dev/null | head -1)
                if [ -f "$timing_json" ]; then
                    python3 -c "import json; print(json.load(open('$timing_json')).get('max_frequency_mhz', 'N/A'))" 2>/dev/null > "$actual_output_dir/freq.txt" || true
                elif [ -f "$actual_output_dir/ibex_optimized_timing_metrics.json" ]; then
                    # Fallback to regular timing file if ODC didn't produce timing
                    python3 -c "import json; print(json.load(open('$actual_output_dir/ibex_optimized_timing_metrics.json')).get('max_frequency_mhz', 'N/A'))" 2>/dev/null > "$actual_output_dir/freq.txt" || true
                fi
            else
                if [ -f "$actual_output_dir/ibex_optimized_timing_metrics.json" ]; then
                    python3 -c "import json; print(json.load(open('$actual_output_dir/ibex_optimized_timing_metrics.json')).get('max_frequency_mhz', 'N/A'))" 2>/dev/null > "$actual_output_dir/freq.txt" || true
                fi
            fi
        fi
...
export EXTRA_ARGS BASE_OUTPUT_DIR VERBOSE RED GREEN YELLOW BLUE NC RUNS_PER_DSL HAS_GATES_FLAG HAS_ODC_FLAG
...
if [ $SUCCESS_COUNT -gt 1 ]; then
    ...
    # Determine what metrics we have based on flags
    CSV_HAS_AREA=false
    CSV_HAS_FREQ=false

    # Check if we have chip area data (from --gates flag)
    if [ "$HAS_GATES_FLAG" = true ]; then
        CSV_HAS_AREA=true
        # Check if any result has frequency data
        for dsl_file in "${DSL_FILES[@]}"; do
            dsl_basename=$(basename "$dsl_file" .dsl)
            if [ "$RUNS_PER_DSL" -gt 1 ]; then
                for run_num in $(seq 1 $RUNS_PER_DSL); do
                    if [ -f "$BASE_OUTPUT_DIR/run_${run_num}/${dsl_basename}/freq.txt" ]; then
                        CSV_HAS_FREQ=true
                        break 2
                    fi
                done
            else
                if [ -f "$BASE_OUTPUT_DIR/${dsl_basename}/freq.txt" ]; then
                    CSV_HAS_FREQ=true
                    break
                fi
            fi
    ...
    # Header - include chip area and/or frequency if available
    if [ "$CSV_HAS_AREA" = true ] && [ "$CSV_HAS_FREQ" = true ]; then
        echo "DSL,Result_Type,Inputs,Outputs,Constraints,Latches,AND_gates,Levels,Total_area_um2,Max_freq_MHz" > "$CSV_FILE"
    elif [ "$CSV_HAS_AREA" = true ]; then
        echo "DSL,Result_Type,Inputs,Outputs,Constraints,Latches,AND_gates,Levels,Total_area_um2" > "$CSV_FILE"
    else
    ...
    for dsl_file in "${DSL_FILES[@]}"; do
        ...
        # Determine which log file to use based on flags
        if [ "$HAS_ODC_FLAG" = true ] && [ -f "$result_dir/odc_optimized_synthesis/abc.log" ]; then
            log_file="$result_dir/odc_optimized_synthesis/abc.log"
        ...
        if [ -f "$log_file" ]; then
            ...
            if [ ! -z "$stats" ]; then
                ...
                # Extract total chip area from area.txt if available
                chip_area="N/A"
                if [ "$CSV_HAS_AREA" = true ] && [ -f "$result_dir/area.txt" ]; then
                    chip_area=$(cat "$result_dir/area.txt" 2>/dev/null || echo "N/A")
                    chip_area=${chip_area:-"N/A"}
                fi

                # Extract frequency from freq.txt if available
                freq="N/A"
                if [ "$CSV_HAS_FREQ" = true ] && [ -f "$result_dir/freq.txt" ]; then
                    freq=$(cat "$result_dir/freq.txt" 2>/dev/null || echo "N/A")
                    freq=${freq:-"N/A"}
                fi

                # Write to CSV based on what columns we have
                if [ "$CSV_HAS_AREA" = true ] && [ "$CSV_HAS_FREQ" = true ]; then
                    echo "$dsl_basename,$result_type,$inputs,$outputs,$constraints,$latches,$and_gates,$levels,$chip_area,$freq" >> "$CSV_FILE"
                elif [ "$CSV_HAS_AREA" = true ]; then
    ...
    echo ""
    if [ "$CSV_HAS_AREA" = true ] && [ "$CSV_HAS_FREQ" = true ]; then
        echo "Quick comparison (sorted by total chip area):"
        # Skip header, replace non-numeric values with large values for sorting, sort by chip area column (9)
        tail -n +2 "$CSV_FILE" | awk -F',' '{
            # If chip area (column 9) is not a number, replace with a large value for sorting
            if ($9 !~ /^[0-9.]+$/) $9 = "999999999";
            print $0;
        }' OFS=',' | sort -t',' -k9 -g | head -20 | \
            awk -F',' 'BEGIN {printf "%-25s %-15s %10s %8s %15s %12s\n", "DSL", "Result_Type", "AND_gates", "Levels", "Total_area(µm²)", "Max_freq(MHz)"}
                       {printf "%-25s %-15s %10s %8s %15s %12s\n", $1, $2, $7, $8, $9, $10}'
    elif [ "$CSV_HAS_AREA" = true ]; then
        echo "Quick comparison (sorted by total chip area):"
        # Skip header, replace non-numeric chip area with a large value, sort by chip area column (9)
        tail -n +2 "$CSV_FILE" | awk -F',' '{
            # If chip area (column 9) is not a number, replace with a large value for sorting
            if ($9 !~ /^[0-9.]+$/) $9 = "999999999";
            print $0;
        }' OFS=',' | sort -t',' -k9 -g | head -20 | \
            awk -F',' 'BEGIN {printf "%-25s %-15s %10s %8s %15s\n", "DSL", "Result_Type", "AND_gates", "Levels", "Total_area(µm²)"}
                       {printf "%-25s %-15s %10s %8s %15s\n", $1, $2, $7, $8, $9}'
    else
        echo "Quick comparison (sorted by AND gates):"
        sort -t',' -k7 -n "$CSV_FILE" | column -t -s',' | head -20
    fi
fi
