...
# (TIMED version: records synlig/techmap, ABC/scorr, ODC/scorr, and gate synthesis times)
#
# Supports multiple cores via YAML config files (Ibex, BOOM, Rocket, CVA6, etc.)
# Usage: ./synth_core_simplified_timed.sh [OPTIONS] <rules.dsl> [output_dir]
...
# Timing variables (timed version)
_TIME_SYNLIG=0
_TIME_ABC_SCORR=0
_TIME_ODC_SCORR=0
_TIME_GATE_SYNTH=0
_TIMING_FILE="$OUTPUT_DIR/synth_timing.csv"
...
# --- TIMING: synlig/techmap start ---
_T_SYNLIG_START=$(date +%s)
...
_T_SYNLIG_END=$(date +%s)
_TIME_SYNLIG=$((_T_SYNLIG_END - _T_SYNLIG_START))
echo "TIMING: synlig/techmap = ${_TIME_SYNLIG}s"
# --- TIMING: synlig/techmap end ---
...
if command -v abc &> /dev/null; then
    ...
    if [ -f "$ABC_INPUT" ] && [ -s "$ABC_INPUT" ]; then
        ...
        _T_ABC_END=$(date +%s)
        _TIME_ABC_SCORR=$((_T_ABC_END - _T_ABC_START))
        echo "TIMING: ABC/scorr = ${_TIME_ABC_SCORR}s"
        # --- TIMING: ABC/scorr end ---
...
PYEOF
                    ...
                    # --- TIMING: ODC ABC/scorr start ---
                    _T_ODC_ABC_START=$(date +%s)
                    ...
                    _T_ODC_ABC_END=$(date +%s)
                    _TIME_ODC_SCORR=$((_T_ODC_ABC_END - _T_ODC_ABC_START))
                    echo "TIMING: ODC ABC/scorr = ${_TIME_ODC_SCORR}s"
                    # --- TIMING: ODC ABC/scorr end ---
                ...
                if [ -f "${BASE}_yosys.aig" ]; then
                    ...
                    # --- TIMING: ODC ABC/scorr (no-ODC baseline) start ---
                    _T_ODC_ABC_START=$(date +%s)
                    ...
                    _T_ODC_ABC_END=$(date +%s)
                    _TIME_ODC_SCORR=$((_T_ODC_ABC_END - _T_ODC_ABC_START))
                    echo "TIMING: ODC ABC/scorr (baseline) = ${_TIME_ODC_SCORR}s"
                    # --- TIMING: ODC ABC/scorr end ---
...
if [ "$SYNTHESIZE_GATES" = true ]; then
    ...
    # --- TIMING: gate synthesis start ---
    _T_GATE_START=$(date +%s)
    ...
    _T_GATE_END=$(date +%s)
    _TIME_GATE_SYNTH=$((_T_GATE_END - _T_GATE_START))
    echo "TIMING: gate synthesis = ${_TIME_GATE_SYNTH}s"
    # --- TIMING: gate synthesis end ---
...
# Write per-synthesis-run timing CSV
echo "=========================================="
echo "Synthesis Timing Summary"
echo "=========================================="
echo "  synlig/techmap:   ${_TIME_SYNLIG}s"
echo "  ABC/scorr:        ${_TIME_ABC_SCORR}s"
echo "  ODC ABC/scorr:    ${_TIME_ODC_SCORR}s"
echo "  Gate synthesis:   ${_TIME_GATE_SYNTH}s"
_TIME_TOTAL=$((_TIME_SYNLIG + _TIME_ABC_SCORR + _TIME_ODC_SCORR + _TIME_GATE_SYNTH))
echo "  Total:            ${_TIME_TOTAL}s"

# Append to timing CSV (create header if file doesn't exist)
if [ ! -f "$_TIMING_FILE" ]; then
    echo "dsl,synlig_techmap_s,abc_scorr_s,odc_scorr_s,gate_synth_s,total_s" > "$_TIMING_FILE"
fi
echo "${DSL_BASENAME},${_TIME_SYNLIG},${_TIME_ABC_SCORR},${_TIME_ODC_SCORR},${_TIME_GATE_SYNTH},${_TIME_TOTAL}" >> "$_TIMING_FILE"
echo "Timing written to: $_TIMING_FILE"
