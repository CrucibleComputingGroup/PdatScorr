#!/bin/bash
# Static timing analysis using OpenSTA and PPDK Standard Library
#
# Usage: ./analyze_timing_ppdk.sh <gate_level.v> [clock_name] [clock_period_ns] [module_name] [output_base]
...
if [ "$#" -lt 1 ] || [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
    ...
    echo ""
    echo "Arguments:"
    echo "  gate_level.v      Gate-level Verilog netlist"
...
SCRIPT_DIR_TIMING="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
PROJECT_ROOT="$(cd "$SCRIPT_DIR_TIMING/../../.." && pwd)"
PPDK_LIB="$PROJECT_ROOT/PPDK_Standard_Library_1.0V_25C_TYP_X1.lib"

if [ -n "$LIBERTY_FILE" ] && [ -f "$LIBERTY_FILE" ]; then
    PPDK_LIB="$LIBERTY_FILE"
fi
...
if {[catch {read_liberty $PPDK_LIB} err]} {
...
if { \$wns != "INFINITY" && \$wns != "-INFINITY" } {
    ...
    puts "  Actual: [format "%.6g" \$max_freq] MHz ([format "%.3f" [expr {1000.0 / \$max_freq}]] ns period)"
...
# Update script with actual module name
...
if [ $STA_EXIT -eq 0 ]; then
    ...
    if [ -n "$WNS" ]; then
        MAX_FREQ=$(python3 -c "wns=float('$WNS'); period=float('$CLK_PERIOD'); max_period = period - wns; print(f'{1000.0/max_period:.6g}' if max_period > 0 else '0')" 2>/dev/null || echo "0")
